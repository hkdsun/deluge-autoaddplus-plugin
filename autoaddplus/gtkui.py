...
class OptionsDialog():
    ...
    def show(self, options={}, watchdir_id=None):
        self.glade = gtk.glade.XML(get_resource("autoaddplus_options.glade"))
    ...
    def on_apply(self, Event=None):
        client.autoaddplus.set_options(str(self.watchdir_id), self.generate_opts()).addCallbacks(self.on_added, self.on_error_show)
    ...
    def on_add(self, Event=None):
        client.autoaddplus.add(self.generate_opts()).addCallbacks(self.on_added, self.on_error_show)
...
class GtkUI(GtkPluginBase):
    def enable(self):
        ...
        sw.show_all()
        component.get("Preferences").add_page("AutoAddPlus", self.glade.get_widget("prefs_box"))
        self.on_show_prefs()


    def disable(self):
        component.get("Preferences").remove_page("AutoAddPlus")
    ...
    def on_remove_button_clicked(self, Event=None):
        ...
        if watchdir_id:
            client.autoaddplus.remove(watchdir_id)
        ...
        if watchdir_id:
            if col and col.get_title() == 'On':
                if self.watchdirs[watchdir_id]['enabled']:
                    client.autoaddplus.disable_watchdir(watchdir_id)
                else:
                    client.autoaddplus.enable_watchdir(watchdir_id)
            else:
    ...
    def on_apply_prefs(self):
        log.debug("applying prefs for AutoAddPlus")
        for watchdir_id, watchdir in self.watchdirs.iteritems():
            client.autoaddplus.set_options(watchdir_id, watchdir)

    def on_show_prefs(self):
        client.autoaddplus.get_config().addCallback(self.cb_get_config)
