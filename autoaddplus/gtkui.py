...
class OptionsDialog(object):
    ...
    def show(self, options=None, watchdir_id=None):
        ...
        self.builder.add_from_file(get_resource('autoaddplus_options.ui'))
    ...
    def on_add(self, event=None):
        try:
            options = self.generate_opts()
            client.autoaddplus.add(options).addCallbacks(self.on_added, self.on_error_show)
        except IncompatibleOption as ex:
            dialogs.ErrorDialog(_('Incompatible Option'), str(ex), self.dialog).run()
...
class GtkUI(Gtk3PluginBase):
    def enable(self):
        ...
        client.register_event_handler(
            'AutoaddPlusOptionsChangedEvent', self.on_options_changed_event
        )
        ...
        component.get('Preferences').add_page(
            _('AutoAddPlus'), self.builder.get_object('prefs_box')
        )

    def disable(self):
        component.get('Preferences').remove_page(_('AutoAddPlus'))
    ...
    def on_show_prefs(self):
        client.autoaddplus.get_watchdirs().addCallback(self.cb_get_config)
